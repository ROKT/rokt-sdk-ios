...
- **A placement still loading is discarded, up to its hand-off to the renderer:** if `clearSession()` is called while a `selectPlacements` call is in flight and its experience has not yet been handed to the renderer, that placement is not shown; it reports `PlacementFailure` instead, and nothing from it is carried into the new session. A placement whose experience has already been handed to the renderer stays visible and is treated like a placement on screen (the bullet above). The hand-off happens just before your event handler receives `RoktEvent.HideLoadingIndicator`, so a placement can be the renderer's a moment before your loading indicator is hidden.
