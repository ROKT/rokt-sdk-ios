...
class RoktInternalImplementation {
    ...
    // Set by clearSession, consumed by the next cache-read decision. The experience cache can
    // satisfy a placement without any network call, and the server is what mints a session — so
    // a cache hit straight after a reset would leave the next customer with no session at all.
    ...
    func replayPendingTxnEvents() {
        ...
        for batch in txnPendingEventStore.drainValid() {
            guard let sessionId = batch.sessionId else {
                // Written by an SDK version that predates session binding. Replaying it would
                // attach those events to whatever session is live now, so drop it instead.
    ...
    ///
    /// Sequencing is the whole point here. `EventQueue.flush()` runs synchronously through to
    /// `TxnEventService` construction, and that service captures the departing session's token
    /// in memory as it is built — so events generated by the customer who is leaving stay
    /// attributed to them. Only then is the persisted session wiped, synchronously, so a
    /// placement started immediately afterwards cannot rehydrate what was just dropped.
    func clearSession() {
        RoktAPIHelper.logApiCalled(Self.apiClearSessionCode)
        EventQueue.flush()
        TxnSessionManager.clearPersistedSession(store: txnSessionStore)
        // Clears the legacy diagnostics session id and fans out to RealTimeEventManager, so the
        // next customer starts with an empty real-time event store.
        sessionManager.invalidateSession()
        // A cached experience was fetched inside the session being dropped, so it goes with it.
        // The clear is asynchronous, hence the one-shot flag as well — see shouldReadFromCache().
    ...
    /// Gate for *reading* the experience cache. Writes are unaffected, so the new customer's
    /// experience is still cached for their own session.
    ///
    /// `clearSession()` both empties the cache and sets this one-shot flag. The flag is what makes
    /// the behaviour deterministic: emptying the cache is asynchronous while the read is a direct
    /// synchronous file read, so a placement issued immediately after a reset could otherwise still
    /// find the departing customer's entry and skip the offers call that mints the new session.
