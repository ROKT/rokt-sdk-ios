#!/usr/bin/env bash
#
# CocoaPods trunk push with retries. Treats trunk "duplicate entry" as success.
# Poll trunk until a pod version is visible (used before publishing dependent podspecs).
#
# Source from the repo root:   . scripts/pod_push.sh
# On GitHub Actions, run `set +e` before calling—the default run shell uses bash -e.
#
