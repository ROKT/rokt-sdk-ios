...
- **StripeAfterpayManager**: Manages redirect-based Afterpay / Clearpay flows via `STPPaymentHandler`; validates `PaymentContext.billingAddress` and confirms the PaymentIntent with the configured return URL — the partner's `universalLinkReturnURL` verbatim, or `<urlScheme>://rokt-payment-return`. The confirmation runs through the extension's own Stripe client and no longer changes the host app's shared `STPAPIClient` (its publishable key or connected account).
